...
export function setSort(key, multi) {
  ...
  if (!multi) {
    const current =
      existingIndex >= 0 ? state.sort[existingIndex].direction : null;
...
export function getVisibleItems() {
  ...
  if (c.dueDateTo) {
    items = items.filter(
      (i) => !i.dueDate || i.dueDate <= c.dueDateTo
    );
  }
...
export function updateItemField(id, key, newValue) {
  ...
  if (key === "estimatedHours") {
    ...
    // Para campos complejos como tags esperamos ya el valor normalizado
...
function isEqualValue(a, b) {
  ...
  // Arrays (por ejemplo, tags)
  if (Array.isArray(a) && Array.isArray(b)) {
    // Comparación superficial basada en JSON; suficiente para arrays pequeños de tags
    try {
